    ...
    public class ChannelingScheduleService : IChannelingSchedule
    {
        ...
        public int GetTotalRefundHospitalFeeCount(int id)
        {
            using (DataAccess.HospitalDBContext dbContext = new DataAccess.HospitalDBContext())
            {
                ...
                // Exclude invoices that have both types of refunds
                var partialRefundInvoiceIds = hospitalFeeRefundInvoiceIds.Except(doctorFeeRefundInvoiceIds).ToList();

                // Return the count of invoices with only hospital fee refunds
                return partialRefundInvoiceIds.Count();
            }
        }
        ...
        public int GetTotalRefundDoctorFeeCount(int id)
        {
            using (DataAccess.HospitalDBContext dbContext = new DataAccess.HospitalDBContext())
            {
                ...
                // Exclude invoices that have both types of refunds
                var partialRefundInvoiceIds = doctorFeeRefundInvoiceIds.Except(hospitalFeeRefundInvoiceIds).ToList();

                // Return the count of invoices with only doctor fee refunds
                return partialRefundInvoiceIds.Count();
            }
        }
